...
Plan memory and disk capacity together.
For persistent nodes, WAL, partition files, snapshots, and the OS page cache are part of the same production budget as heap, page memory, checkpoint buffers, and JVM native memory.

image::images/memory-budget-capacity-process.svg[Ignite memory and disk capacity budget]

With persistence enabled, a write first changes pages in memory and is persisted to the WAL.
Later, checkpointing writes dirty pages from memory to partition files.
This means that write-heavy workloads need enough data region memory, checkpoint buffer space, WAL capacity, partition-file capacity, and OS page cache headroom.

image::images/memory-budget-capacity-persistence.svg[Ignite persistence capacity path]

Capacity limits fail in different ways.
Heap, direct memory, metaspace, and thread-stack pressure usually surfaces as `OutOfMemoryError`.
Data region and checkpoint-buffer pressure usually surfaces as `IgniteOutOfMemoryException`.
